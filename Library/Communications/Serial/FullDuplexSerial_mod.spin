...
DPG 30-Apr-08:
  Added support for GPS data.
DPG 1-May-08:
  Added function gettarget(), used to get the target latitude and longitude.
...
'DPG 30-APR-08
  byte buff[80],Rcv',cksum
  byte readstr[80]
  long Null[1]
  long flag[20]
  long count, strlen, strlenflag
...
PUB readln |i
  i := 0
  bytefill(@buff,0,80)      'need to handle/understand buffers better
  strlen := strsize(@buff)
  dec(strlen)
  repeat
   Rcv := rx
...
pub copy_buffer ( buffer,args)| ptr,arg,j
         bytemove(buffer,@buff,count) '  copy received data to buffer
         ptr := buffer
         arg := 0
...
pub gettarget|temp,i ''Used to recieve target Latitude and Longitude
  i := 0
  count := 0
  temp := readln
  strlen := strsize(@temp)
  repeat i from 0 to strlen
    if temp[i] == ","
      i++
    if temp[i] == ","
      repeat count from 0 to i
        targetLAT[count] := temp[count]
    if temp[i] == ","
      repeat count from 0 to i
        targetLONG[count] := temp[count]
  target[0] := targetLAT
  target[1] := targetLONG
  return target  
